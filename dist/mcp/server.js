...
app.post("/api/mcp/call", async (req, res) => {
    ...
    try {
        if (tool === "generate_workflow")
            ...
            res.json({ result: { url: await createLogicArtAdapter({ serverUrl: "https://logic.art" }).visualize(params.workflow) } });
        else if (tool === "council_query")
            res.json({ result: await council.query(params) });
        else if (tool === "save_workflow")
            res.json({ result: await saveWorkflow(params.workflow) });
        else if (tool === "load_workflow")
            res.json({ result: await loadWorkflow(params.id) });
        else if (tool === "list_workflows")
            res.json({ result: await listWorkflows() });
        else if (tool === "web_search")
            res.json({ result: await toolHandlers.web_search(params) });
        else if (tool === "summarize")
            res.json({ result: await toolHandlers.summarize(params) });
        else
